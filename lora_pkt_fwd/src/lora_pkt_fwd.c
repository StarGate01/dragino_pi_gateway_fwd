...
static int sx1276_txpw = 20;
static char server_type[16] = "server_type";
static char board[16] = "PG301";
static FILE *fp = NULL;
...
static int parse_gateway_configuration(const char * conf_file) {
    ...
    if (str != NULL) {
        ...
        MSG_DEBUG(DEBUG_INFO, "INFO~ GW mode is configured to \"%s\"\n", server_type);
    }

    str = json_object_get_string(conf_obj, "board");
    if (str != NULL) {
        strncpy(board, str, sizeof board);
        MSG_DEBUG(DEBUG_INFO, "INFO~ GW board is configured to \"%s\"\n", board);
    }

    val = json_object_get_value(conf_obj, "sx1276");
    if (json_value_get_type(val) == JSONBoolean) {
        sx1276 = (bool)json_value_get_boolean(val);
        MSG_DEBUG(DEBUG_INFO, "INFO~ GW download stream use SX127x module\n");
    }
...
int main(void)
{
    ...
    char *global_cfg_path= "/etc/lora-gateway/global_conf.json"; /* contain global (typ. network-wide) configuration */
    char *local_cfg_path = "/etc/lora-gateway/local_conf.json"; /* contain node specific configuration, overwrite global parameters for parameters that are defined in both */
    char *debug_cfg_path = "/etc/lora-gateway/debug_conf.json"; /* if present, all other configuration files are ignored */
    ...
    if ((!strcmp(board, "PG08P") || !strcmp(board, "LG08P")) && sx1276) {
        ...
        sxradio->dio[1] = 6;
        sxradio->dio[2] = 8;
        sxradio->rf_power = (sx1276_txpw > 0 && sx1276_txpw <= 20) ? sx1276_txpw : 16; /* sx1276 power < 20 */
