...
Check their manual on how to install all the dependecies and this gateway.
...
Known issue: running the packet forwarder with using the service sometimes results in incorret GPS data. Running manually 
using `./lora_pkt_fwd/lora_pkt_fwd` works perfectelly. High change that it has to do with the user who is executing the script
...
### 3.2. packet_forwarder/lora_pkt_fwd/update_gwid.sh
